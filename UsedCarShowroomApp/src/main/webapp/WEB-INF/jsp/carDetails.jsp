	...
	<div class="container mt-5" align="center">
		...
		<table class="table table-striped table-hover uppercase mt-5 mb-5 border">
			...
			<c:forEach var="sh" items="${car.serviceHistories}">
			  <tr><form:form action="${contextPath}/car/${car.carId}/save-service-history" method="post" modelAttribute="serviceHistory">
			  	<td>
			  	  <c:choose>
					<c:when test="${serviceHistoryId == sh.id}">
						<form:hidden path="id"/>
			  	...
			  	<td>
			  	  <sec:authorize access="hasRole('ADMIN')">
					  ...
					  <c:otherwise>
			  	  		<a href="${contextPath}/car/${car.carId}/edit-service-history/${sh.id}"><i title="Edit Service History" class="fa fa-edit text-info ml-2"></i></a>
					  </c:otherwise>
					</c:choose>
			  	  	<a href="${contextPath}/car/${car.carId}/delete-service-history/${sh.id}" onclick="return confirm('Are you sure you want to delete?')"><i title="Delete Service History" class="fa fa-trash text-danger ml-2"></i></a>
			  	  </sec:authorize>
			  	</td>
			  </form:form></tr>
			</c:forEach>
